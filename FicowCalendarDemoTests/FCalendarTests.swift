...
final class FCalendarTests: XCTestCase {
    ...
    func testDateComputationPerformance() {
        self.measure {
            let startDate = CalendarTestHelper.yyyyMMddDateFromString("0001-01-01")!
            let endDate = CalendarTestHelper.yyyyMMddDateFromString("2200-01-01")!
            calendar = FCalendar()
            dataSource = MockFCalendarDataSource(start: startDate,
                                                  end: endDate)
            calendar.dataSource = dataSource
            ...
            let lastSection = calendar.numberOfSections - 1
            _ = calendar.dateOfFirstDayInSection(lastSection)
        }
    }
